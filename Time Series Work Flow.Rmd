...
The name of the column column you are interested in should be formatted as a 
string and pointing to "column".
...
data <- PBS %>%
  filter(ATC2 == "A10") %>%
  select(Month, Scripts, Cost) %>%
  summarize(Total_Cost = sum(Cost), Total_Scripts = sum(Scripts))
...
train_test <- train_test_split(data, 'monthly')
...
# Un-comment if using transformed version
# data['transform'] <- transform_comparison(data, column, train_stats, append = TRUE)
# test['transform'] <- transform_comparison(test, column, train_stats, append = TRUE)
...
Un-comment below code to henceforth the transformed data to the "column" variable.
```{r}
# column <- 'transform'
```
...
Function uses MAPE to sort the dataframe and return best performing benchmark.
```{r}
plot_best_benchmark_residuals <- function(model_accuracy, dataframe, column) {
  col_name <- as.name(column)
  
  benchmark_data <- model_accuracy %>% arrange(MAPE)
...
arima_fit <- train %>%
  model(auto = ARIMA(!!col_name, stepwise=FALSE, approx=FALSE))
```

```{r}
arima_fc <- arima_fit %>%
    forecast(h=dim(test)[1])
